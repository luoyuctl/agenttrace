...
var translations = map[string]map[Lang]string{
	...
	"report_possible_cost_drivers": {
		EN: "Possible cost drivers",
		ZH: "可能费用驱动",
	},
	"report_evidence": {
		EN: "Evidence",
	...
	"diff_field_tokens":        {EN: "Tokens", ZH: "Token"},
	...
	"list_reason":           {EN: "Reason", ZH: "原因"},
	"list_reason_cost":      {EN: "cost", ZH: "费用"},
	"list_reason_duration":  {EN: "duration", ZH: "耗时"},
	"list_reason_failure":   {EN: "failure", ZH: "失败"},
	"list_reason_hanging":   {EN: "hanging", ZH: "卡住"},
	...
	"driver_summary_title":  {EN: "TOP DRIVERS", ZH: "主要驱动"},
	"driver_source":         {EN: "source", ZH: "来源"},
	"driver_model":          {EN: "model", ZH: "模型"},
	"driver_anomaly":        {EN: "anomaly", ZH: "异常"},
	"driver_none":           {EN: "none", ZH: "无"},
	"driver_jump_hint":      {EN: "S/M/A jumps to the top source, model, or anomaly group", ZH: "S/M/A 跳到最高来源、模型或异常分组"},
	"driver_counts":         {EN: "visible %d · parsed %d · discovered %d", ZH: "可见 %d · 已解析 %d · 已发现 %d"},
	...
	"help_overview_short":   {EN: "$ top · ! crit · / find · ? keys", ZH: "$ 高费 · ! 严重 · / 搜索 · ? 键位"},
	...
	"cmd_filter_anomaly_type":    {EN: "anomaly filter %q", ZH: "异常筛选 %q"},
	...
	"insight_authority_suffix": {EN: ", authority %s", ZH: "，权限 %s"},
	...
	"diag_slow_summary_title": {EN: "SLOW-RUN SUMMARY", ZH: "慢运行摘要"},
	"diag_slow_path_title":    {EN: "SLOW PATH", ZH: "慢路径"},
	"diag_slow_primary_tool_latency": {
		EN: "Most likely driver: tool latency",
		ZH: "最可能原因: 工具延迟",
	},
	"diag_slow_primary_gap": {
		EN: "Most likely driver: long gap",
		ZH: "最可能原因: 长间隔",
	},
	"diag_slow_primary_loop": {
		EN: "Most likely driver: retry loop",
		ZH: "最可能原因: 重试循环",
	},
	"diag_slow_primary_params": {
		EN: "Most likely driver: large parameters",
		ZH: "最可能原因: 大参数",
	},
	"diag_slow_primary_context": {
		EN: "Most likely driver: context pressure",
		ZH: "最可能原因: 上下文压力",
	},
	"diag_slow_primary_clear": {
		EN: "No dominant slow-run driver",
		ZH: "未发现主要慢运行原因",
	},
	"diag_slow_tool_detail": {EN: "%s P95 %.1fs, max %.1fs", ZH: "%s P95 %.1fs，最大 %.1fs"},
	"diag_slow_gap_detail":  {EN: "longest gap %.1fs between events", ZH: "事件间最长间隔 %.1fs"},
	"diag_slow_loop_detail": {
		EN: "repeated tool results or loop cost can hide progress",
		ZH: "重复工具结果或循环成本可能掩盖进展",
	},
	"diag_slow_params_detail": {
		EN: "large tool arguments can add latency or context bloat",
		ZH: "大型工具参数可能增加延迟或上下文膨胀",
	},
	"diag_slow_clear_detail": {
		EN: "latency, gaps, retries, parameters, and context look normal",
		ZH: "延迟、间隔、重试、参数和上下文看起来正常",
	},
	"diag_slow_no_evidence":        {EN: "No slow-path evidence yet", ZH: "暂无慢路径证据"},
	"diag_slow_path_tool":          {EN: "Slow tool: %s (P95 %.1fs, max %.1fs)", ZH: "慢工具: %s (P95 %.1fs，最大 %.1fs)"},
	"diag_slow_path_gap":           {EN: "Long gap: %.1fs between events", ZH: "长间隔: 事件间 %.1fs"},
	"diag_slow_path_loop":          {EN: "Retry loop: repeated tool output or loop waste", ZH: "重试循环: 重复工具输出或循环浪费"},
	"diag_slow_path_params":        {EN: "Large params: %d heavy tool call(s)", ZH: "大参数: %d 次重型工具调用"},
	"diag_slow_path_context":       {EN: "Context pressure: %s", ZH: "上下文压力: %s"},
	"cost_driver_context_pressure": {EN: "possible driver: context pressure (%s risk, %.1f%% estimated utilization)", ZH: "可能驱动: 上下文压力 (%s 风险，估算利用率 %.1f%%)"},
	"cost_driver_large_params":     {EN: "possible driver: %d large tool parameter/output event(s)", ZH: "可能驱动: %d 次大型工具参数/输出事件"},
	"cost_driver_retry_loop":       {EN: "possible driver: retry loop or repeated tool output evidence", ZH: "可能驱动: 重试循环或重复工具输出证据"},
	"cost_driver_tool_failures":    {EN: "possible driver: %d/%d failed tool result(s) (%.1f%%)", ZH: "可能驱动: %d/%d 次工具结果失败 (%.1f%%)"},
	"cost_driver_high_tokens_per_turn": {
		EN: "possible driver: %d tokens per assistant turn across %d turn(s)",
		ZH: "可能驱动: %d tokens/助手轮，共 %d 轮",
	},
	...
	"diff_verdict_title":            {EN: "DIFF VERDICT", ZH: "差异结论"},
	...
	"diff_verdict_slower_duration":  {EN: "B is slower mainly because duration increased.", ZH: "B 更慢，主要因为时长增加。"},
	"diff_verdict_faster_duration":  {EN: "B is faster because duration decreased.", ZH: "B 更快，因为时长下降。"},
	"diff_verdict_costlier":         {EN: "B is costlier mainly because estimated cost increased.", ZH: "B 更贵，主要因为估算费用增加。"},
	"diff_verdict_cheaper":          {EN: "B is cheaper because estimated cost decreased.", ZH: "B 更便宜，因为估算费用下降。"},
	"diff_verdict_failures":         {EN: "B is weaker because tool failures increased.", ZH: "B 更弱，因为工具失败增加。"},
	"diff_verdict_tokens":           {EN: "B uses more context because tokens increased.", ZH: "B 使用更多上下文，因为 Token 增加。"},
	"diff_verdict_tools":            {EN: "B changed path because tool calls increased.", ZH: "B 路径变重，因为工具调用增加。"},
	"diff_verdict_model":            {EN: "Model changed; compare quality and cost before judging.", ZH: "模型已变化，需同时看质量和费用。"},
	"diff_verdict_health":           {EN: "B is lower quality because health dropped.", ZH: "B 质量更低，因为健康分下降。"},
	"diff_verdict_similar":          {EN: "No dominant slowdown or cost driver stands out.", ZH: "没有明显的变慢或费用驱动因素。"},
	...
	"metric_tokens_short":       {EN: "tok", ZH: "tok"},
	...
	"metric_errors_short":       {EN: "err", ZH: "错"},
	"metric_error_rate":         {EN: "ERROR RATE", ZH: "错误率"},
	"metric_p95":                {EN: "P95", ZH: "P95"},
	"metric_p95_latency":        {EN: "P95 LATENCY", ZH: "P95 延迟"},
	"metric_elapsed":            {EN: "ELAPSED", ZH: "耗时"},
	"metric_elapsed_time":       {EN: "ELAPSED TIME", ZH: "总耗时"},
	"metric_all_sessions":       {EN: "all sessions", ZH: "全部会话"},
	"metric_top_agent":          {EN: "TOP AGENT", ZH: "最高 Agent"},
	"metric_top_model":          {EN: "TOP MODEL", ZH: "最高模型"},
	"metric_health":             {EN: "HEALTH", ZH: "健康"},
	"metric_health_short":       {EN: "h", ZH: "健"},
	...
	"panel_inspect_first":       {EN: "INSPECT FIRST", ZH: "优先查看"},
	...
	"triage_panel_aside":        {EN: "open", ZH: "打开"},
	"triage_no_sessions":        {EN: "No sessions loaded yet", ZH: "尚未加载会话"},
	"triage_issue_label":        {EN: "Issue", ZH: "问题"},
	"triage_impact_label":       {EN: "Impact", ZH: "影响"},
	"triage_evidence_label":     {EN: "Evidence", ZH: "证据"},
	"triage_next_label":         {EN: "Next", ZH: "下一步"},
	"triage_key_critical":       {EN: "press ! then Enter", ZH: "按 ! 再 Enter"},
	"triage_key_anomalies":      {EN: "press a then Enter", ZH: "按 a 再 Enter"},
	"triage_key_cost":           {EN: "press $ then Enter", ZH: "按 $ 再 Enter"},
	"triage_key_detail":         {EN: "press Enter", ZH: "按 Enter"},
	"inspect_top_cost":          {EN: "Top cost", ZH: "最高费用"},
	"inspect_slowest":           {EN: "Slowest", ZH: "最慢会话"},
	"inspect_critical":          {EN: "Critical", ZH: "严重会话"},
	"inspect_no_critical":       {EN: "No critical sessions", ZH: "暂无严重会话"},
	...
	"compact_focus":             {EN: "INSPECT FIRST", ZH: "优先查看"},
	...
	"waste_suggestion_label": {EN: "Suggestion", ZH: "建议"},
	...
	"search_results_title":       {EN: "Search results", ZH: "搜索结果"},
	"search_no_results":          {EN: "No matching session metadata found.", ZH: "未找到匹配的会话元数据。"},
	"search_health":              {EN: "health=%d", ZH: "健康=%d"},
	"search_cwd":                 {EN: "cwd", ZH: "工作目录"},
	"search_path":                {EN: "path", ZH: "路径"},
	"search_match_name":          {EN: "name", ZH: "名称"},
	"search_match_path":          {EN: "path", ZH: "路径"},
	"search_match_cwd":           {EN: "cwd", ZH: "工作目录"},
	"search_match_source":        {EN: "source", ZH: "来源"},
	"search_match_model":         {EN: "model", ZH: "模型"},
	"search_match_authority":     {EN: "authority", ZH: "权限"},
	"search_match_cost_driver":   {EN: "possible cost driver", ZH: "可能费用驱动"},
	"search_match_tool":          {EN: "tool", ZH: "工具"},
	"search_match_tool_arg":      {EN: "tool argument", ZH: "工具参数"},
	"search_match_file":          {EN: "file", ZH: "文件"},
	"search_match_anomaly":       {EN: "anomaly", ZH: "异常"},
	"search_match_large_param":   {EN: "large parameter", ZH: "大参数"},
	"search_match_tool_warning":  {EN: "tool warning", ZH: "工具警告"},
	...
	"doctor_cache_detail":        {EN: "%d parsed session cache entries, %d reusable for this scan, %d cached directory listings", ZH: "%d 条已解析会话缓存记录，本次扫描可复用 %d 条，%d 个目录列表缓存"},
	...
	"doctor_next_cache":          {EN: "No reusable parsed session entries for this scan. Cached directory listings may still speed discovery; the next TUI startup should reuse parsed sessions incrementally.", ZH: "本次扫描没有可复用的已解析会话缓存。目录列表缓存仍可能加速发现；下一次启动 TUI 应会增量复用已解析会话。"},
	...
	"gate_evidence_title":        {EN: "Local evidence:\n", ZH: "本地证据:\n"},
	"gate_evidence_avg_health":   {EN: "- avg health: %.1f\n", ZH: "- 平均健康分: %.1f\n"},
	"gate_evidence_critical":     {EN: "- critical sessions: %d\n", ZH: "- 严重会话数: %d\n"},
	"gate_evidence_tool_fail_rate": {
		EN: "- tool fail rate: %.1f%%\n",
		ZH: "- 工具失败率: %.1f%%\n",
	},
	"gate_evidence_report":  {EN: "- report: %s\n", ZH: "- 报告: %s\n"},
	"gate_evidence_session": {EN: "- lowest-health session: %s\n", ZH: "- 最低健康分会话: %s\n"},
	"gate_evidence_next":    {EN: "- inspect: `%s`\n", ZH: "- 排查命令: `%s`\n"},
